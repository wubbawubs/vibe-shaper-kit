...
const notifications = [
  ...
  { title: "New application", message: "David R. applied via LinkedIn" },
];
...
export const ProductHeroAnimation = () => {
  ...
  // Stage flow animation - cycles through all 4 stages including Offer
  useEffect(() => {
  ...
  return (
    <div 
      className="relative w-full h-[420px] md:h-[480px] bg-gradient-to-br from-muted/40 via-background to-muted/20 rounded-2xl border border-border/50 overflow-hidden shadow-xl"
      aria-hidden="true"
    >
      {/* Premium glow orbs */}
      ...
      <div className="absolute inset-0 p-6 md:p-8">
        ...
        <div className="space-y-3 max-w-2xl mx-auto">
          {candidates.map((candidate, i) => (
            <motion.div
              ...
              transition={{ delay: prefersReducedMotion ? 0 : 0.3 + i * 0.15, duration: 0.5 }}
              whileHover={prefersReducedMotion ? {} : { scale: 1.01, x: 4 }}
              className="bg-background/90 backdrop-blur-sm rounded-xl border border-border/50 p-4 flex items-center justify-between shadow-sm hover:shadow-md transition-shadow cursor-pointer group"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs text-muted-foreground font-medium">{stages[activeStage]}</span>
        ...
        <AnimatePresence mode="wait">
          {showNotification && (
            <motion.div
              ...
              className="absolute bottom-6 right-4 md:right-6 bg-primary text-primary-foreground rounded-xl px-4 md:px-5 py-3 md:py-4 shadow-xl max-w-[260px] md:max-w-[240px]"
            >
        ...
        <motion.div 
          className="absolute bottom-6 left-4 md:left-6 flex items-center gap-2"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
